 ...
 * @notice The Ticket extends the standard ERC20 and ControlledToken interfaces
 *         with time-weighted average balance functionality. The average
 *         balance held by a user between two timestamps can be calculated, as
 *         well as the historic balance. The historic total supply is available
 *         as well as the average total supply between two timestamps. A user
 *         may "delegate" their balance; increasing another user's historic
 *         balance while retaining their tokens.
 */
contract Ticket is ControlledToken, Constants, ITicket {
    ...
    /// @notice Record of tickets total supply and ring buff parameters used for
    ///         observation.
    TwabLib.Account internal totalSupplyTwab;

    /// @notice Mapping of delegates. Each address can delegate their ticket
    ///         power to another.
     ...
     * @param _controller ERC20 ticket controller address (ie: Prize Pool
     *                    address).
     */
    function initialize(
        string memory _name,
    ...
    function getAccountDetails(
        address _user
    ) external view override returns (TwabLib.AccountDetails memory) {
    ...
    function getTwab(
        address _user,
        uint16 _index
    ) external view override returns (ObservationLib.Observation memory) {
    ...
    function getBalancesAt(
        ...
        require(
            _targets.length <= MAX_TIMESTAMPS_LENGTH,
    ...
    function _getAverageBalancesBetween(
        ...
        require(
            _startTimes.length <= MAX_TIMESTAMPS_LENGTH,
            "Ticket/wrong-array-length"
        );
    ...
    function _beforeTokenTransfer(
        ...
        if (_from != address(0) && _to != address(0)) {
            IPrizePool(controller).updateUserRewardAndFormer(
                _from,
                balanceOf(_from),
    ...
    function _afterTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (balanceOf(from) == 0) {
            (bool exist, uint index) = _isHolderExist(from);
            if (exist) {
                _removeHolder(index);
            }
        }
    ...
    function _removeHolder(uint _index) internal {
        require(_index < holders.length, "Ticket/remove-holder-index-out-of-bound");
        holders[_index] = holders[holders.length - 1];
        holders.pop();
    }

    function _addHolder(address _user) internal {
        (bool exist,) = _isHolderExist(_user);
        if (!exist) {
            holders.push(_user);
        }
    }

    function _isHolderExist(address _user) internal view returns(bool, uint) {
